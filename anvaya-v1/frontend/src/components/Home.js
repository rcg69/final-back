...
function Home() {
  ...
  useEffect(() => {
    const fetchScratchCards = async () => {
      try {
        // UPDATED: Use full backend URL from environment variable for API call
  ...
  const BannerSlider = ({ images, interval = 6000 }) => {
    ...
    // UPDATED: Responsive - track window width to dynamically update sizing
    ...
    const bannerStyle = {
      position: "relative",
      // UPDATED: width adjusts — full viewport for phones, container width for desktops
      width: windowWidth <= 600 ? "100vw" : "100%",
      maxWidth: "100vw",
      // UPDATED: height adapts to window width for responsive banner size
